...
class ContentGenerator:
    ...
    def _upload_to_cloudinary(self, video_file: Path, title: str) -> Optional[str]:
    ...
    def _clear_all_cloudinary_videos(self) -> bool:
        """Delete all videos from Cloudinary Reels folder to ensure complete cleanup."""
        try:
            if not self.cloudinary_api_key or not self.cloudinary_api_secret:
                logger.error("Cloudinary API credentials not found")
                return False
            ...
            # Delete everything inside the folder (fixed folder mode)
            try:
                folder_path = "Reels"  # your folder path
...
If you are the rightful owner of any content used and have any concerns, please contact me. I will promptly remove or credit the content as requested."""
    ...
    def find_video_urls(self) -> List[str]:
        """
        Find YouTube video URLs based on multiple search queries.
        ...
        for query in self.search_queries:
            ...
            try:
                with self._get_driver() as driver:
                    ...
                    while len(video_urls) < self.video_count and scroll_attempts < max_scroll_attempts:
                        video_elements = driver.find_elements(By.CSS_SELECTOR, 'a#video-title')
                        
                        for element in video_elements:
                            href = element.get_attribute('href')
                            if href and href not in video_urls and href not in all_video_urls:
                                video_urls.append(href)
                        
                        if len(video_urls) < self.video_count:
                            driver.execute_script("window.scrollTo(0, document.documentElement.scrollHeight);")
                            time.sleep(2)
                            scroll_attempts += 1
                    
                    self._log_substep(f"Query '{query}': Found {len(video_urls)} videos", "✅")
                    all_video_urls.extend(video_urls)
                    
            except Exception as e:
                self._log_step("ERROR", f"Failed finding videos for '{query}': {e}", "❌")
    ...
    def download_video(self, url: str) -> bool:
        """
        Download a single video using yt-dlp and upload to Google Drive.
    ...
    def download_video(self, url: str) -> bool:
        ...
        try:
            ...
            self._log_substep("Downloading from YouTube...", "⬇️")
            cmd = [
                "yt-dlp",
                "-o", output_template,
                "--no-warnings",
            ...
            if result.returncode == 0:
                ...
                if not downloaded_files:
                    self._log_substep("Could not locate downloaded file on disk", "❌")
                    return False
                
                video_file = downloaded_files[0]
                
                # Upload to Cloudinary
                self._log_substep("Uploading to Cloudinary for temporary hosting...", "☁️")
                cloudinary_url = self._upload_to_cloudinary(video_file, title)
                ...
                self._log_substep(f"Download failed: {result.stderr[:100]}...", "❌")
                return False
...
def main():
    ...
    try:
        ...
        config = {
            "search_queries": ["Roblox Adventure"],
            "video_count": 2
        }
